...
"""
    ...
    def generate_blog_post_from_history(self, title_instruction):
        ...
        prompt = f"""
당신은 나와 대화했던 내용을 바탕으로 전문적이고 통찰력 있는 기술/일상 블로그 포스트를 작성하는 시니어 테크니컬 에디터입니다.
...
1. VitePress 기반 블로그이므로 마크다운 최상단에 반드시 Frontmatter(yaml 형식)를 포함할 것.
   ...
   - tags: [AI, 대화요약, Tech]
2. 대화 내용을 서론-본론-결론의 짜임새 있는 구조로 정리할 것.
3. **[기술적 디테일 강화]** 대화 중에 등장한 기술적인 내용을 단순 요약하지 말고 최대한 상세하게 작성할 것.
   - **문제 상황 및 배경:** 어떤 기술적 이슈나 고민이 있었는지 명확히 서술할 것.
   - **해결 과정 (Troubleshooting):** 에러 메시지, 디버깅 과정, 그리고 특정 아키텍처나 코드를 선택한 기술적 이유(Why & How)를 깊이 있게 분석할 것.
   - **코드 및 문법 강조:** 코드 스니펫이 있다면 반드시 마크다운 코드 블록(```언어)을 사용하여 구문 강조를 적용하고, 핵심이 되는 코드 라인에 대한 부연 설명을 추가할 것.
   - 전문 용어는 인라인 코드(`)를 적극적으로 활용하여 테크 블로그다운 가독성을 확보할 것.
4. 오직 마크다운 텍스트 결과물만 출력할 것 (인사말이나 부연 설명 금지).
